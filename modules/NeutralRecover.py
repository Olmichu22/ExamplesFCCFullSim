...
class ParticleWHits:
    ...
    def separate_hits_by_detector(self):
        ...
        for type_hit, hit_coords in self.associated_hits.items():
            if type_hit == DETECTOR_ID_TO_TYPE['INNER_TRACKER']:
                self.hits_tracker = hit_coords
            elif type_hit == DETECTOR_ID_TO_TYPE['ECAL']:
                self.hits_ecal = hit_coords
            elif type_hit == DETECTOR_ID_TO_TYPE['HCAL']:
                self.hits_hcal = hit_coords
            elif type_hit == DETECTOR_ID_TO_TYPE['MUON_TRACKER']:
    ...
    def get_numpy_hits(self):
        ...
        return {"tracker": tracker, "ecal": ecal, "hcal": hcal, "muon": muon}
...
def debug_reco_tau(reco_tau, hit_type_map):
    ...
    for i, const in constituents.items():
        cluster = const.getClusters()[0] if const.getClusters() else None
        if cluster:
